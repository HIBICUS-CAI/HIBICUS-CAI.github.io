...
- 特定のSystemとComponentより呼び出す、特定なコードを実行するの部分、関数ポインタでフレームワークにレジスター・紐付けています
...
- コンポーネントベースシステム & スクリプトに似てる感じのコード実行仕組み
    ...
    一つのEntityに異なる機能が持っているComponentを添付するより、実行の時必要な挙動をさせることができ、加えて必要性より随時追加・削除もできます。継承よりの機能実装と比べると、再利用可能、管理しやすい、データをファイルに保存しやすい等の優位性を持っています。
