...
- 絶対必要なベースツール
- フレームワーク担当機能により利用している中間ツール・ファイル
- HycFrame2Dフレームワーク自体
- フレームワークに紐つけているファイル
...
- 特定のSystemとComponentにより呼び出す、特定なコードを実行するの部分、関数ポインタでフレームワークにレジスター・紐付けています
...
- コンポーネントベースシステム & スクリプトに似てる感じのコード実行仕組み
    ...
    一つのEntityに異なる機能が持っているComponentを添付するにより、実行の時必要な挙動をさせることができ、加えて必要性により随時追加・削除もできます。継承よりの機能実装と比べると、再利用可能、管理しやすい、データをファイルに保存しやすい等の優位性を持っています。
    ...
    でもこれは悪い点ではないと思います。このやり方により、全てのゲームシステムに関する処理をInteract・Inputコンポーネントに集中でき、矛盾がない関数も再利用できるようにしました。例えば
    ```
    Interact処理A = 関数1 + 関数2
...
- JSONシーンファイルによりのデータ管理
    ...
    それと、JSON化したもう一つの利点としては、もし将来こういうシーン編集処理をGUI化する必要があったら、従来難しい操作をJSONファイルの操作に変換することで、手間を取らずに実装できると考えています。
...
- マルチスレッドでのシーン管理
    ...
    でもその後私は、Entity-Componentと組合せば、どれぐらいのEntityが作れたを確認できることを気づいました。つまりローディングの進み具合も画面に表示できるようになりました。なお、これでダイナミックなローディング画面作成も可能になり、より面白いゲームを作れると思います。
...
- HycFrame3Dに解決済
  ...
  - `new`でメモリ確保するによりの低い実行速度

    このフレームワークのEntityとComponentは全部`new`で直接確保されています。処理は簡単ですが、CPUキャッシュをうまく利用できず、実行速度が減られると考えています。

    [HycFrame3D](hycframe3d_jp.md)は同種類のComponentを一つの`vector`にまとめて、連続のデータ構造によりCPUキャッシュ利用率を向上することができます。
